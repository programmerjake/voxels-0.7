...
protected:
    ...
    {
        return Generate::unitBox(TextureDescriptor(),
                                 td,
                                 TextureDescriptor(),
                                 TextureDescriptor(),
                                 TextureDescriptor(),
...
protected:
    FallingFullBlock(std::wstring name,
        ...
        : FallingBlock(name,
                       BlockShape(VectorF(0.5f), VectorF(0.5f)),
                       lightProperties,
    ...
    FallingFullBlock(std::wstring name,
                     ...
                     bool isFaceBlockedNZ,
                     bool isFaceBlockedPZ,
                     TextureDescriptor tdNX,
                     TextureDescriptor tdPX,
                     TextureDescriptor tdNY,
                     TextureDescriptor tdPY,
                     TextureDescriptor tdNZ,
                     TextureDescriptor tdPZ,
                     RenderLayer rl)
        : FallingBlock(name,
                       BlockShape(VectorF(0.5f), VectorF(0.5f)),
                       lightProperties,
    ...
    FallingFullBlock(std::wstring name,
                     ...
                     RenderLayer rl = RenderLayer::Opaque)
        : FallingFullBlock(name,
                           lightProperties,
                           blockRayCollisionMask,
                           areFacesBlocked,
                           areFacesBlocked,
...
public:
                                                  ...
                                                  RayCasting::Ray ray) const override
    {
        ...
        std::tuple<bool, float, BlockFace> collision = ray.getAABoxEnterFace(
        ...
        {
            collision = ray.getAABoxExitFace((VectorF)blockIterator.position(),
                                             (VectorF)blockIterator.position() + VectorF(1));
            if(!std::get<0>(collision) || std::get<1>(collision) < RayCasting::Ray::eps)
                return RayCasting::Collision(world);
