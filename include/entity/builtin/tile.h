...
public:
    typedef void (BlockDescriptor::*MoveHandlerType)(Block b,
                                                     BlockIterator bi,
                                                     World &world,
                                                     WorldLockManager &lock_manager,
                                                     double deltaTime) const;
    typedef std::atomic_bool *(BlockDescriptor::*AttachHandlerType)(
        Block b, BlockIterator bi, World &world, WorldLockManager &lock_manager) const;
...
public:
    explicit TileEntity(BlockDescriptorPointer block,
                        MoveHandlerType moveHandler,
                        AttachHandlerType attachHandler)
    ...
    void addToWorld(World &world,
                    ...
                    PositionI position,
                    Block block,
                    std::atomic_bool *hasEntity = nullptr) const
    {
        world.addEntity(this,
                        (PositionF)position,
                        VectorF(0),
                        lock_manager,
                        std::shared_ptr<void>(new TileData(block, position, hasEntity)));
    }
    virtual void write(PositionF positionf,
                       VectorF velocity,
    ...
    virtual std::shared_ptr<void> read(PositionF positionf,
                                       VectorF velocity,
