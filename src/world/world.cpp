...
{
    const std::size_t lightingThreadCount = roundUpThreadCount(
        getProcessorCount() / 2);
    const std::size_t blockUpdateThreadCount = roundUpThreadCount(
        getProcessorCount() / 2);
#if 1 || defined(NDEBUG)
    const std::size_t generateThreadCount = roundUpThreadCount(
        getProcessorCount() / 2);
#else
...
private:
    struct Chunk final
    {
        ...
        std::list<PositionI>::iterator chunksListIterator;bool empty = true;
        Chunk()
            : columns(),
              chunksListIterator()
        {
    ...
    Chunk &getChunk(PositionI pos, RandomSource &randomSource)
    {
        ...
        if(retval.empty)
        {
            ...
            for(int dx = 0; dx < BlockChunk::chunkSizeX; dx++)
            {
                ...
                {
                    retval.columns[dx][dz] = randomSource.getBiomeProperties(
                        pos + VectorI(dx, 0, dz));
                }
            }
            retval.chunksListIterator = chunksList.insert(chunksList.begin(),
                pos);
        }
        else
        {
            chunksList.splice(chunksList.begin(), chunksList,
                retval.chunksListIterator);
        }
        return retval;
    }
...
public:
    ...
    void fillWorldChunk(World &world, PositionI chunkBasePosition,
        WorldLockManager &lock_manager, RandomSource &randomSource)
    {
        Chunk &c = getChunk(chunkBasePosition, randomSource);
        world.setBiomePropertiesRange<false>(chunkBasePosition,
            chunkBasePosition
                + VectorI(BlockChunk::chunkSizeX - 1,
                    BlockChunk::chunkSizeY - 1, BlockChunk::chunkSizeZ - 1),
            lock_manager, c.columns, VectorI(0, 0, 0));
    }
};
...
private:
    ...
    Chunk &getChunk(PositionI pos, T generateFn)
    {
        ...
        if(retval.empty)
        {
            ...
            thread_local_variable<BlocksGenerateArray, BlocksTLSTag>blocks(
                TLS::getSlow());
                ...
                for(int dy = 0; dy < BlockChunk::chunkSizeY; dy++)
                {
                    for(int dz = 0; dz < BlockChunk::chunkSizeZ; dz++)
                    {
                        retval.blocks[dx][dy][dz] =
                            static_cast<PackedBlock>(std::move(
                                blocks.get()[dx][dy][dz]));
...
public:
    ...
    template<typename T>
    void getChunk(PositionI chunkBasePosition, BlocksGenerateArray &blocks,
        checked_array<checked_array<int, BlockChunk::chunkSizeZ>,
            BlockChunk::chunkSizeX>&groundHeights, T generateFn)
    {
        ...
        for(int dx = 0; dx < BlockChunk::chunkSizeX; dx++)
        {
            for(int dy = 0; dy < BlockChunk::chunkSizeY; dy++)
            {
                ...
                {
                    blocks[dx][dy][dz] =
                        static_cast<Block>(c.blocks[dx][dy][dz]);
                }
            }
        }
        for(int dx = 0; dx < BlockChunk::chunkSizeX; dx++)
...
private:
    void generateGroundChunkH(BlocksGenerateArray &blocks,
        checked_array<checked_array<int, BlockChunk::chunkSizeZ>,
            BlockChunk::chunkSizeX>&groundHeights, PositionI chunkBasePosition,
        ...
        RandomSource &randomSource, const std::atomic_bool *abortFlag) const
    {
        ...
        for(int cx = 0; cx < BlockChunk::chunkSizeX; cx++)
        {
            for(int cz = 0; cz < BlockChunk::chunkSizeZ; cz++)
            {
                ...
                PositionI columnBasePosition = chunkBasePosition
                    + VectorI(cx, 0, cz);
                bi.moveTo(columnBasePosition, lock_manager);
                ...
                for(const BiomeWeights::value_type &v : bp.getWeights())
                {
                    ...
                    groundHeightF += weight
                        * biome->getGroundHeight(columnBasePosition,
                            randomSource);
                }
                int groundHeight = ifloor(0.5f + groundHeightF);
                groundHeights[cx][cz] = groundHeight;
                bp.getDominantBiome()->makeGroundColumn(chunkBasePosition,
                    columnBasePosition, blocks, randomSource, groundHeight);
                for(int cy = 0; cy < BlockChunk::chunkSizeY; cy++)
                {
                    ...
                    if(pos.y <= World::SeaLevel)
                    {
                            ...
                            == Blocks::builtin::Air::descriptor())
                        {
                            ...
                            {
                                block.lighting =
                                    waterLightProperties.calculateTransmittedLighting(
                                        block.lighting);
                                ...
                                != nullptr)
                        {
                            ...
                            if(surfaceBlock.descriptor
                                == Blocks::builtin::Grass::descriptor())
                            {
                                surfaceBlock = Block(
                                    Blocks::builtin::Dirt::descriptor());
                            }
                        }
                    }
                    else
        ...
        RandomSource &randomSource, const std::atomic_bool *abortFlag) const
    {
        ...
        thread_local_variable<std::shared_ptr<GroundChunksCache>,
            PGroundCacheTag>pGroundCache(lock_manager.tls, nullptr);
        struct SeedTag
        {
        };
        thread_local_variable<World::SeedType, SeedTag>seed(lock_manager.tls,
            0);
        ...
        pBiomesCache.get()->fillWorldChunk(world, chunkBasePosition,
            lock_manager, randomSource);
        checkForAbort(abortFlag);
        pGroundCache.get()->getChunk(chunkBasePosition, blocks, groundHeights,
    ...
    std::vector<std::shared_ptr<const DecoratorInstance>> generateDecoratorsInChunk(
        DecoratorDescriptorPointer descriptor, PositionI chunkBasePosition,
        WorldLockManager &lock_manager, World &world,
        RandomSource &randomSource, const std::atomic_bool *abortFlag) const
    {
        struct PBlocksTag
        ...
        typedef checked_array<checked_array<int, BlockChunk::chunkSizeZ>,
            BlockChunk::chunkSizeX>GroundHeightsType;
        struct PGroundHeightsTag
        {
        };
        thread_local_variable<std::unique_ptr<GroundHeightsType>,
            PGroundHeightsTag>pGroundHeights(lock_manager.tls);
        if(!pGroundHeights.get())
            pGroundHeights.get().reset(new GroundHeightsType);
        GroundHeightsType &groundHeights = *pGroundHeights.get();
        generateGroundChunk(blocks, groundHeights, chunkBasePosition,
            lock_manager, world, randomSource, abortFlag);
        ...
        rg.discard(30);
        BlockIterator chunkBaseIterator = world.getBlockIterator(
            chunkBasePosition, lock_manager.tls);
        for(int x = 0; x < BlockChunk::chunkSizeX; x++)
        {
            for(int z = 0; z < BlockChunk::chunkSizeZ; z++)
            {
                ...
                for(const BiomeWeights::value_type &v : bp.getWeights())
                {
                    BiomeDescriptorPointer biome = std::get<0>(v);
                    float weight = std::get<1>(v);
                    generateCountF += weight
                        * biome->getChunkDecoratorCount(descriptor);
                }
                generateCountF /= BlockChunk::chunkSizeX
                    * BlockChunk::chunkSizeZ;
                int generateCount = ifloor(
                    std::generate_canonical<float, 1000>(rg) + generateCountF);
                VectorI relativeColumnSurfacePosition = columnBasePosition
                    + VectorI(0, World::SeaLevel, 0) - chunkBasePosition;
                while(relativeColumnSurfacePosition.y >= 0
                    && relativeColumnSurfacePosition.y
                        < BlockChunk::chunkSizeY - 1)
                {
                    const Block &b =
                        blocks[relativeColumnSurfacePosition.x][relativeColumnSurfacePosition.y][relativeColumnSurfacePosition.z];
                    if(!b.good())
                        break;
                ...
                PositionI columnSurfacePosition = relativeColumnSurfacePosition
                    + chunkBasePosition;
                for(int i = 0; i < generateCount; i++)
                {
                    auto instance = descriptor->createInstance(
                        chunkBasePosition, columnBasePosition,
                        columnSurfacePosition, lock_manager, chunkBaseIterator,
                        blocks, randomSource, decoratorGenerateNumber++);
                    if(instance != nullptr)
...
protected:
    virtual void generate(PositionI chunkBasePosition,
        BlocksGenerateArray &blocks, World &world,
        WorldLockManager &lock_manager, RandomSource &randomSource,
        const std::atomic_bool *abortFlag) const override
    {
        ...
        thread_local_variable<std::shared_ptr<DecoratorCache>,
            PDecoratorCacheTag>pDecoratorCache(lock_manager.tls, nullptr);
        struct LastSeedTag
        {
        };
        thread_local_variable<World::SeedType, LastSeedTag>lastSeed(
            lock_manager.tls, 0);
        ...
        for(DecoratorDescriptorPointer descriptor : DecoratorDescriptors)
        {
            ...
            for(VectorI i = VectorI(-chunkSearchDistance, 0, 0);
                i.x <= chunkSearchDistance; i.x++)
            {
                for(i.z = -chunkSearchDistance; i.z <= chunkSearchDistance;
                    i.z++)
                {
                    checkForAbort(abortFlag);
                    VectorI rcpos = VectorI(BlockChunk::chunkSizeX,
                        BlockChunk::chunkSizeY, BlockChunk::chunkSizeZ) * i;
                    PositionI pos = rcpos + chunkBasePosition;
                    auto instances = decoratorCache.getChunkInstances(pos,
                        descriptor);
                    if(!std::get<1>(instances))
                    {
                        decoratorCache.setChunkInstances(pos, descriptor,
                            generateDecoratorsInChunk(descriptor, pos,
                                lock_manager, world, randomSource, abortFlag));
                        instances = decoratorCache.getChunkInstances(pos,
                            descriptor);
                    }
                    for(std::shared_ptr<const DecoratorInstance> instance : std::get<
                        0>(instances))
                    {
                        checkForAbort(abortFlag);
                        instance->generateInChunk(chunkBasePosition,
                            lock_manager, world, blocks);
                    }
                }
            }
        }
...
BlockUpdate *World::removeAllBlockUpdatesInChunk(BlockUpdateKind kind,
    BlockIterator bi, WorldLockManager &lock_manager)
{
    std::shared_ptr<BlockChunk> chunk = bi.chunk;
    lock_manager.clear();
    BlockChunkFullLock lockChunk(*chunk);
    auto lockIt = std::unique_lock<
        decltype(chunk->getChunkVariables().blockUpdateListLock)>(
        chunk->getChunkVariables().blockUpdateListLock);
    BlockUpdate *retval = nullptr;
    std::size_t &blockUpdatesLeft =
        bi.chunk->getChunkVariables().blockUpdatesPerPhase[BlockUpdateKindPhase(
            kind)];
    for(BlockUpdate *node = chunk->getChunkVariables().blockUpdateListHead;
        node != nullptr;)
    {
        if(node->kind == kind)
        {
            if(node->chunk_prev != nullptr)
                ...
                chunk->getChunkVariables().blockUpdateListHead =
                    node->chunk_next;
            if(node->chunk_next != nullptr)
                node->chunk_next->chunk_prev = node->chunk_prev;
            else
                chunk->getChunkVariables().blockUpdateListTail =
                    node->chunk_prev;
    ...
    BlockIterator bi, WorldLockManager &lock_manager)
{
    ...
    if(chunk->getChunkVariables().lastBlockUpdateTimeValid)
        deltaTime = static_cast<float>(std::chrono::duration_cast<
    ...
    BlockUpdate *retval = nullptr;
    std::size_t &blockUpdatesLeft =
        bi.chunk->getChunkVariables().blockUpdatesPerPhase[phase];
    for(BlockUpdate *node = chunk->getChunkVariables().blockUpdateListHead;
        node != nullptr;)
    {
      ...
      blockUpdateDidAnything(false)
{
    ...
    ([this, abortFlag, &tls]()
    {
        ...
        for(;;)
        {
            ...
            bool isGoodStartingPosition = true;
            const std::int32_t searchDistance = 20;
            for(PositionI pos(-searchDistance, 0, -searchDistance, Dimension::Overworld); pos.x <= searchDistance;pos.x++)
            {
                for(pos.z = -searchDistance; pos.z <= searchDistance; pos.z++)
                {
                    BiomeProperties bp =
                    randomSource.getBiomeProperties(pos);
                    if(!bp.getDominantBiome()->isGoodStartingPosition())
                    {
                        isGoodStartingPosition = false;
                        break;
                    }
                }
                if(!isGoodStartingPosition)
                {
                    break;
                }
            }
...
World::~World()
{
    ...
    {
        LockedPlayers lockedPlayers = players().lock();
        copiedPlayerList.assign(lockedPlayers.begin(), lockedPlayers.end()); // hold another reference to players so we
...
Lighting World::getBlockLighting(BlockIterator bi,
    WorldLockManager &lock_manager, bool isTopFace)
{
...
void World::lightingThreadFn(TLS &tls)
{
    ...
    while(!destructing)
    {
            ...
            chunkIter++)
        {
            ...
            for(BlockUpdate *node = removeAllBlockUpdatesInChunk(
                BlockUpdateKind::Lighting, cbi, lock_manager); node != nullptr;
                node = removeAllBlockUpdatesInChunk(BlockUpdateKind::Lighting,
                    cbi, lock_manager))
            {
                didAnything = true;
                std::size_t litBlockCount = 0;
                while(node != nullptr)
                {
                    BlockIterator bi = cbi;
                    bi.moveTo(node->position, lock_manager);
                    Block b = bi.get(lock_manager);
                    if(b.good())
                    {
                        BlockIterator binx = bi;
                        binx.moveTowardNX(lock_manager);
                        BlockIterator bipx = bi;
                        bipx.moveTowardPX(lock_manager);
                        BlockIterator biny = bi;
                        biny.moveTowardNY(lock_manager);
                        BlockIterator bipy = bi;
                        bipy.moveTowardPY(lock_manager);
                        BlockIterator binz = bi;
                        binz.moveTowardNZ(lock_manager);
                        BlockIterator bipz = bi;
                        bipz.moveTowardPZ(lock_manager);
                        if(litBlockCount++ > 500)
                        {
                            litBlockCount = 0;
                            lock_manager.clear();
                        }
                        Lighting newLighting =
                            b.descriptor->lightProperties.eval(
...
void World::blockUpdateThreadFn(TLS &tls, bool isPhaseManager)
{
    ...
    while(!destructing)
    {
        ...
        if(isPhaseManager)
        {
            ...
            blockUpdateCurrentPhase = BlockUpdatePhaseNext(
                blockUpdateCurrentPhase);
            ...
            chunkIter++)
        {
            ...
            for(BlockUpdate *node = removeAllReadyBlockUpdatesInChunk(phase,
                cbi, lock_manager); node != nullptr; node =
                removeAllReadyBlockUpdatesInChunk(phase, cbi, lock_manager))
            {
                didAnything = true;
                while(node != nullptr)
                {
                    ...
                    if(b.good())
                    {
                        b.descriptor->tick(*this, b, bi, lock_manager,
                            node->kind);
                    }
...
void World::generateChunk(std::shared_ptr<BlockChunk> chunk,
    WorldLockManager &lock_manager, const std::atomic_bool *abortFlag,
    std::unique_ptr<ThreadPauseGuard> &pauseGuard)
{
    lock_manager.clear();
    std::size_t lockManagerUseCount = 0;
    pauseGuard = nullptr;
    WorldLockManager new_lock_manager(lock_manager.tls, 1000);
    World newWorld(worldGeneratorSeed, nullptr, internal_construct_flag());
    worldGenerator->generateChunk(chunk->basePosition, newWorld,
        new_lock_manager, abortFlag);
#if 0
    ...
    typedef checked_array<
        checked_array<BiomeProperties, BlockChunk::chunkSizeZ>,
    ...
    BlockIterator gcbi = newWorld.getBlockIterator(chunk->basePosition,
        new_lock_manager.tls);
    for(auto i = BlockChunkRelativePositionIterator::begin();
        i != BlockChunkRelativePositionIterator::end(); i++)
    {
        BlockIterator bi = gcbi;
        bi.moveBy(*i, new_lock_manager);
    ...
    for(int dx = 0; dx < BlockChunk::chunkSizeX; dx++)
    {
        ...
        {
            BlockIterator bi = gcbi;
            bi.moveBy(VectorI(dx, 0, dz), new_lock_manager);
    ...
    BlockIterator cbi = getBlockIterator(chunk->basePosition, lock_manager.tls);
    {
        ...
        WrappedEntity::ChunkListType &srcChunkList =
            gcbi.chunk->getChunkVariables().entityList;
        WrappedEntity::ChunkListType &destChunkList =
            cbi.chunk->getChunkVariables().entityList;
        for(WrappedEntity::ChunkListType::iterator srcChunkIter =
            srcChunkList.begin(); srcChunkIter != srcChunkList.end();)
        {
            BlockChunkSubchunk *srcSubchunk = srcChunkIter->currentSubchunk;
            assert(srcSubchunk);
            new_lock_manager.block_biome_lock.set(srcSubchunk->lock);
            WrappedEntity::SubchunkListType &srcSubchunkList =
                srcSubchunk->entityList;
            ...
            if(!srcChunkIter->entity.good())
            {
                srcSubchunkList.erase(
                    WrappedEntity::SubchunkListType::const_iterator(
                        srcSubchunkIter));
                srcChunkIter = srcChunkList.erase(
                    WrappedEntity::ChunkListType::const_iterator(srcChunkIter));
                continue;
            }
            srcChunkIter->lastEntityRunCount = 0;
            PositionF position =
                srcChunkIter->entity.physicsObject->getPosition();
            VectorF velocity =
                srcChunkIter->entity.physicsObject->getVelocity();
            srcChunkIter->entity.physicsObject->destroy();
            srcChunkIter->entity.physicsObject =
                srcChunkIter->entity.descriptor->makePhysicsObject(
            ...
            destBi.moveTo((PositionI)position, lock_manager);
            if(lockManagerUseCount++ > 500)
            {
                lockManagerUseCount = 0;
                lock_manager.clear();
            }
            if(!destBi.tryUpdateLock(lock_manager))
            {
                ...
                destBi.updateLock(lock_manager, lock_range.begin(),
                    lock_range.end());
                new_lock_manager.block_biome_lock.adopt(srcSubchunk->lock);
            }
            WrappedEntity::SubchunkListType &destSubchunkList =
                destBi.getSubchunk().entityList;
            srcChunkIter->currentChunk = destBi.chunk.get();
            srcChunkIter->currentSubchunk = &destBi.getSubchunk();
            destChunkList.splice(destChunkList.end(), srcChunkList,
                srcChunkIter);
            destSubchunkList.splice(destSubchunkList.end(), srcSubchunkList,
                srcSubchunkIter);
    ...
    setBlockRange(chunk->basePosition,
        chunk->basePosition
            + VectorI(BlockChunk::chunkSizeX - 1, BlockChunk::chunkSizeY - 1,
                BlockChunk::chunkSizeZ - 1), lock_manager, *blocks, VectorI(0));
    lock_manager.clear();
}
...
bool World::isInitialGenerateChunk(PositionI position)
{
    if(position == PositionI(0, 0, 0, Dimension::Overworld)
        || position
            == PositionI(-BlockChunk::chunkSizeX, 0, 0, Dimension::Overworld)
        || position
            == PositionI(0, 0, -BlockChunk::chunkSizeZ, Dimension::Overworld)
        || position
            == PositionI(-BlockChunk::chunkSizeX, 0, -BlockChunk::chunkSizeZ,
                Dimension::Overworld))
    {
        return true;
    }
...
void World::chunkGeneratingThreadFn(
    ...
    TLS &tls)
{
    std::unique_ptr<ThreadPauseGuard> pauseGuard = std::unique_ptr<
        ThreadPauseGuard>(new ThreadPauseGuard(*this));
    WorldLockManager lock_manager(tls);
    BlockChunkMap *chunks = &physicsWorld->chunks;
    while(!destructing
        && (!initialChunkGenerateStruct
            ...
            || !*initialChunkGenerateStruct->abortFlag))
    {
            ...
            chunkIter++)
        {
            ...
            if(chunkIter.is_locked())
                chunkIter.unlock();
            bool isCurrentChunkInitialGenerate = isInitialGenerateChunk(
                chunk->basePosition);
            BlockIterator cbi(chunk, chunks, chunk->basePosition, VectorI(0));
            float currentChunkPriority = getChunkGeneratePriority(cbi,
                lock_manager);
            if(!isCurrentChunkInitialGenerate
                && std::isnan(currentChunkPriority))
                continue;
            if(!haveChunk
                || (isCurrentChunkInitialGenerate && !isChunkInitialGenerate)
        ...
        if(haveChunk)
        {
            ...
            true)) // someone else got this chunk
                continue;
            getDebugLog() << L"generating " << chunk->basePosition << L" "
                          << chunkPriority << postnl;
            didAnything = true;

            if(isChunkInitialGenerate && initialChunkGenerateStruct->abortFlag)
                generateChunk(chunk, lock_manager,
                    initialChunkGenerateStruct->abortFlag, pauseGuard);
            else
                ...
                if(initialChunkGenerateStruct->count > 0)
                {
                    ...
                    if(initialChunkGenerateStruct->count == 0)
                    {
                        ...
                        while(initialChunkGenerateStruct->generatorWait)
                            initialChunkGenerateStruct->generatorWaitDoneCond.wait(
                                lockIt);
                        lockIt.unlock();
                        lockIt.release();
                        initialChunkGenerateStruct = nullptr;
...
float World::getChunkGeneratePriority(BlockIterator bi,
    WorldLockManager &lock_manager) // low values mean high priority, NAN means don't generate
{
    ...
    PositionF chunkMaxCorner = chunkMinCorner
        + VectorI(BlockChunk::chunkSizeX, BlockChunk::chunkSizeY,
            BlockChunk::chunkSizeZ);
    ...
    for(ViewPoint *viewPoint : viewPoints)
    {
        ...
        PositionF posXZ = pos;
        posXZ.y = 0;
        float distSquared = boxDistanceSquared(chunkMinCornerXZ,
            chunkMaxCornerXZ, posXZ);
        if(pos.d != bi.position().d
            || distSquared > 2.0f * (float)viewDistance * viewDistance)
            continue;
...
std::shared_ptr<BlockChunk> &getLoadIntoChunk(TLS &tls)
{
    ...
    thread_local_variable<std::shared_ptr<BlockChunk>, retval_tls_tag>retval(
        tls);
    return retval.get();
}
}
...
BlockIterator World::getBlockIteratorForWorldAddEntity(PositionI pos, TLS &tls)
{
    ...
    // world move
    {
        return BlockIterator(chunk, &physicsWorld->chunks, chunk->basePosition,
            BlockChunk::getChunkRelativePosition(pos));
    }
}

Entity *World::addEntity(EntityDescriptorPointer descriptor, PositionF position,
    VectorF velocity, WorldLockManager &lock_manager,
    std::shared_ptr<void> entityData)
{
    ...
    WrappedEntity *entity = new WrappedEntity(
        Entity(descriptor, nullptr, entityData));
    entity->entity.physicsObject = descriptor->makePhysicsObject(entity->entity,
    ...
    WrappedEntity::ChunkListType &chunkList =
        bi.chunk->getChunkVariables().entityList;
    ...
    while(!destructing)
    {
            ...
            chunkIter++)
        {
            ...
            bool isChunkCloseEnough =
                isChunkCloseEnoughToPlayerToGetRandomUpdates(
                    chunkIter->basePosition);
            ...
            if(chunkIter.is_locked())
                chunkIter.unlock();
            BlockIterator cbi(chunk, chunks, chunk->basePosition, VectorI(0));
            std::size_t ranRandomTickCount = 0;
            if(isGenerated && isChunkCloseEnough)
            {
                double fRandomTickCount = deltaTime
                    * (20.0f * 3.0f / 16.0f / 16.0f / 16.0f
                        * BlockChunk::chunkSizeX * BlockChunk::chunkSizeY
                        * BlockChunk::chunkSizeZ);
                // fRandomTickCount *= 5;
                int randomTickCount = static_cast<int>(std::floor(
                    fRandomTickCount
                        + std::generate_canonical<float, 20>(
                            getRandomGenerator())));
                for(int i = 0; i < randomTickCount; i++)
                {
                    VectorI relativePosition = VectorI(
                        std::uniform_int_distribution<>(0,
                            BlockChunk::chunkSizeX - 1)(getRandomGenerator()),
                        std::uniform_int_distribution<>(0,
                            BlockChunk::chunkSizeY - 1)(getRandomGenerator()),
                        std::uniform_int_distribution<>(0,
                            BlockChunk::chunkSizeZ - 1)(getRandomGenerator()));
                    BlockIterator bi = cbi;
                    bi.moveBy(relativePosition, lock_manager);
                    if(ranRandomTickCount++ > 500)
                    {
                        ranRandomTickCount = 0;
                        lock_manager.clear();
                    }
                    Block b = bi.get(lock_manager);
                    if(b.good())
            ...
            while(i != chunkEntityList.end())
            {
                ...
                {
                    lock_manager.block_biome_lock.set(
                        entity.currentSubchunk->lock);
                    WrappedEntity::SubchunkListType &subchunkEntityList =
                        entity.currentSubchunk->entityList;
                    subchunkEntityList.erase(
                        subchunkEntityList.to_iterator(&entity));
                ...
                BlockIterator destBi = cbi;
                destBi.moveTo(
                    (PositionI)entity.entity.physicsObject->getPosition(),
                    lock_manager);
                entity.entity.descriptor->moveStep(entity.entity, *this,
                    lock_manager, deltaTime);
                ...
                WrappedEntity::ChunkListType &chunkDestEntityList =
                    chunk->getChunkVariables().entityList;
                chunkDestEntityList.splice(chunkDestEntityList.end(),
                    chunkEntityList, i);
...
RayCasting::Collision World::castRayCheckForEntitiesInSubchunk(BlockIterator bi,
    RayCasting::Ray ray, WorldLockManager &lock_manager,
    float maxSearchDistance, const Entity *ignoreEntity)
{
    PositionI subchunkPos = BlockChunk::getSubchunkBaseAbsolutePosition(
        bi.position());
    ...
    for(int dx = -1; dx <= 1; dx++)
    {
        for(int dy = -1; dy <= 1; dy++)
        {
            ...
            {
                PositionI currentSubchunkPos = subchunkPos
                    + VectorI(dx * BlockChunk::subchunkSizeXYZ,
                        dy * BlockChunk::subchunkSizeXYZ,
                        dz * BlockChunk::subchunkSizeXYZ);
                bi.moveTo(currentSubchunkPos, lock_manager);
                bi.updateLock(lock_manager);
                for(WrappedEntity &entity : bi.getSubchunk().entityList)
                {
                    ...
                    RayCasting::Collision currentCollision =
                        entity.entity.descriptor->getRayCollision(entity.entity,
                            *this, ray);
...
RayCasting::Collision World::castRay(RayCasting::Ray ray,
    WorldLockManager &lock_manager, float maxSearchDistance,
    ...
    const Entity *ignoreEntity)
{
    ...
    for(auto i = RayCasting::makeRayBlockIterator(ray);
        ...
        i++)
    {
        bi.moveTo(std::get<1>(*i), lock_manager);
        if(&bi.getSubchunk() != lastSubchunk)
        {
            lastSubchunk = &bi.getSubchunk();
            RayCasting::Collision currentCollision =
                castRayCheckForEntitiesInSubchunk(bi, ray, lock_manager,
                    maxSearchDistance, ignoreEntity);
            if(currentCollision < retval)
                retval = currentCollision;
        }
        Block b = bi.get(lock_manager);
        if(!b.good())
        {
            if(retval.valid() && retval.t < std::get<0>(*i)
                && retval.t <= maxSearchDistance)
                return retval;
            return RayCasting::Collision(*this);
        }
        if((b.descriptor->blockRayCollisionMask & blockRayCollisionMask) == 0)
            continue;
        RayCasting::Collision currentCollision = b.descriptor->getRayCollision(
            b, bi, lock_manager, *this, ray);
        if(currentCollision < retval)
            retval = currentCollision;
        if(retval.valid() && retval.t < maxSearchDistance)
...
void World::generateParticlesInSubchunk(BlockIterator sbi,
    WorldLockManager &lock_manager, double currentTime, double deltaTime,
    std::vector<PositionI> &positions)
{
    ...
    for(PositionI pos : positions)
    {
        ...
        bi.moveTo(pos, lock_manager);
        Block b = bi.get(lock_manager);
        if(b.good())
            b.descriptor->generateParticles(*this, b, bi, lock_manager,
...
void World::particleGeneratingThreadFn(TLS &tls)
{
    ...
    while(!destructing)
    {
        ...
        for(std::size_t viewPointIndex = 0;
            viewPointIndex < viewPointsVector.size(); viewPointIndex++)
        {
            PositionF centerPosition = std::get<0>(
                viewPointsVector[viewPointIndex]);
            float generateDistance = std::get<1>(
                viewPointsVector[viewPointIndex]);
            ...
            PositionI minP = BlockChunk::getSubchunkBaseAbsolutePosition(
                (PositionI)minPositionF);
            PositionI maxP = BlockChunk::getSubchunkBaseAbsolutePosition(
                (PositionI)maxPositionF);
            for(PositionI pos = minP; pos.x <= maxP.x; pos.x +=
                BlockChunk::subchunkSizeXYZ)
            {
                    ...
                    BlockChunk::subchunkSizeXYZ)
                {
                        ...
                        BlockChunk::subchunkSizeXYZ)
                    {
                        ...
                        {
                            PositionF centerPosition2 = std::get<0>(
                                viewPointsVector[i]);
                            float generateDistance2 = std::get<1>(
                                viewPointsVector[i]);
                            PositionF minPositionF2 = centerPosition2
                                - VectorF(generateDistance2);
                            PositionF maxPositionF2 = centerPosition2
                                + VectorF(generateDistance2);
                            PositionI minP2 =
                                BlockChunk::getSubchunkBaseAbsolutePosition(
                        ...
                        sbi.moveTo(pos, lock_manager);
                        generateParticlesInSubchunk(sbi, lock_manager,
                            currentTime, deltaTime, positionsBuffer);
                    }
                }
            }
        }
...
void World::invalidateBlockRange(BlockIterator blockIterator, VectorI minCorner,
    VectorI maxCorner, WorldLockManager &lock_manager)
{
    BlockChunkSubchunk *lastSubchunk = nullptr;
    ...
    VectorI maxSubchunk = BlockChunk::getSubchunkBaseAbsolutePosition(
        maxCorner);
    for(VectorI sp = minSubchunk; sp.x <= maxSubchunk.x; sp.x +=
        BlockChunk::subchunkSizeXYZ)
    {
        for(sp.y = minSubchunk.y; sp.y <= maxSubchunk.y; sp.y +=
            BlockChunk::subchunkSizeXYZ)
        {
            for(sp.z = minSubchunk.z; sp.z <= maxSubchunk.z; sp.z +=
                BlockChunk::subchunkSizeXYZ)
            {
                PositionI chunkBasePosition = BlockChunk::getChunkBasePosition(
                    PositionI(sp, blockIterator.position().d));
                IndirectBlockChunk &ibc =
                    physicsWorld->chunks[chunkBasePosition];
                if(!ibc.isLoaded())
                    continue;
                BlockIterator bi = blockIterator;
                bi.moveTo(sp, lock_manager);
                VectorI currentMinCorner = sp;
                VectorI currentMaxCorner = sp
                    + VectorI(BlockChunk::subchunkSizeXYZ - 1);
                ...
                std::size_t invalidatedBlockCount = 0;
                for(VectorI p = currentMinCorner; p.x <= currentMaxCorner.x;
                    p.x++)
                {
                    for(p.y = currentMinCorner.y; p.y <= currentMaxCorner.y;
                        p.y++)
                    {
                        for(p.z = currentMinCorner.z; p.z <= currentMaxCorner.z;
                            p.z++)
                        {
                            BlockIterator biXYZ = bi;
                            biXYZ.moveTo(p, lock_manager);
                            if(invalidatedBlockCount++ > 500)
                            {
                                invalidatedBlockCount = 0;
                                lock_manager.clear();
                                if(lastSubchunk)
                                {
                                    lastSubchunk->invalidate();
                                    lockIt.unlock();
                                }
                                lastSubchunk = nullptr;
                            }
                            ...
                            if(&biXYZ.chunk->getChunkVariables() != lastChunk)
                            {
                                if(lastChunk)
                                    lastChunk->invalidate();
                                lastChunk = &biXYZ.chunk->getChunkVariables();
                            }
                            ...
                            for(BlockUpdateKind kind : enum_traits<
                                BlockUpdateKind>())
                            {
                                ...
                                BlockUpdateKind>())
                            {
                                ...
                                {
                                    BlockUpdate *pnode = BlockUpdate::allocate(
                                        lock_manager.tls, kind,
                                        biXYZ.position(), 0.0f,
                                        blockOptionalData->updateListHead);
                                    bi.chunk->getChunkVariables().blockUpdatesPerPhase[BlockUpdateKindPhase(
                                        kind)]++;
                                    ...
                                    if(biXYZ.chunk->getChunkVariables().blockUpdateListHead
                                        != nullptr)
                                        biXYZ.chunk->getChunkVariables().blockUpdateListHead->chunk_prev =
                                            pnode;
                                    else
...
void World::write(stream::Writer &writerIn, WorldLockManager &lock_manager)
{
    ...
    try
    {
        ...
        stream::MemoryWriter memWriter;
        {
            ...
            {
                for(auto chunkIter = chunksMap->begin();
                    chunkIter != chunksMap->end(); chunkIter++)
                {
            ...
            for(std::shared_ptr<BlockChunk> chunk : chunks)
            {
                ...
                std::vector<WrappedEntity *> entities;
                {
                    ...
                    WrappedEntity::ChunkListType &chunkEntityList =
                        chunk->getChunkVariables().entityList;
                    for(auto i = chunkEntityList.begin();
                        i != chunkEntityList.end(); ++i)
                    {
                        WrappedEntity &entity = *i;
                        if(!entity.entity.good())
                        {
                ...
                entities.clear();
                BlockIterator cbi(chunk, chunksMap, chunk->basePosition,
                    VectorI(0, 0, 0));
                for(std::size_t x = 0; x < BlockChunk::chunkSizeX; x++)
                {
                    for(std::size_t z = 0; z < BlockChunk::chunkSizeZ; z++)
                    {
                        ...
                        columnBlockIterator.moveBy(VectorI(x, 0, z),
                            lock_manager);
                        stream::write<BiomeProperties>(writer,
                            columnBlockIterator.getBiomeProperties(
                                lock_manager));
                        BlockIterator bi = columnBlockIterator;
                        blockUpdates.clear();
                        for(std::size_t y = 0; y < BlockChunk::chunkSizeY;
                            y++, bi.moveTowardPY(lock_manager))
                        {
                            stream::write<Block>(writer, bi.get(lock_manager));
                            for(BlockUpdateIterator iter = bi.updatesBegin(
                                lock_manager);
                                iter != bi.updatesEnd(lock_manager); ++iter)
                            {
                                blockUpdates.emplace_back(iter->getPosition(),
                                    iter->getTimeLeft(), iter->getKind());
                            }
                        }
                        stream::write<std::uint32_t>(writer,
                            static_cast<std::uint32_t>(blockUpdates.size()));
                        for(auto update : blockUpdates)
                        {
                            stream::write<PositionI>(writer,
                                std::get<0>(update));
                            stream::write<float32_t>(writer,
                                std::get<1>(update));
                            stream::write<BlockUpdateKind>(writer,
                                std::get<2>(update));
                        }
                    }
                }
        ...
        lock_manager.clear();
        writerIn.writeBytes(memWriter.getBuffer().data(),
            memWriter.getBuffer().size());
    }
...
{
    std::shared_ptr<std::uint32_t> version = reader.getAssociatedValue<
        std::uint32_t, file_version_tag_t>();
    assert(version);
    return *version;
}
...
std::shared_ptr<World> World::read(stream::Reader &readerIn)
{
    ...
    if(fileVersion < 5)
    {
        throw stream::InvalidDataValueException(
            "old file version not supported");
    }
    ...
    std::shared_ptr<World> retval = std::make_shared<World>(worldGeneratorSeed,
        MyWorldGenerator::getInstance(), internal_construct_flag());
    World &world = *retval;
    ...
    thread_local_variable<BlocksGenerateArray, BlocksTLSTag>blocksTLS(
        lock_manager.tls);
    ...
    thread_local_variable<
        checked_array<checked_array<BiomeProperties, BlockChunk::chunkSizeZ>,
            BlockChunk::chunkSizeX>, BiomesTLSTag>biomesTLS(lock_manager.tls);
    auto &biomes = biomesTLS.get();
    auto blockUpdates = checked_array<
        checked_array<
            checked_array<
                std::vector<std::tuple<PositionI, float, BlockUpdateKind>>,
                BlockChunk::subchunkCountZ>, BlockChunk::subchunkCountY>,
    ...
    for(std::uint64_t chunkIndex = 0; chunkIndex < chunkCount; chunkIndex++)
    {
        ...
        if(chunkBasePosition
            != BlockChunk::getChunkBasePosition(chunkBasePosition))
            throw stream::InvalidDataValueException(
                "block chunk base is not a valid position");
        ...
        for(std::uint64_t entityIndex = 0; entityIndex < entityCount;
            entityIndex++)
        {
            Entity().read(reader,
        ...
        for(std::size_t x = 0; x < BlockChunk::chunkSizeX; x++)
        {
            for(std::size_t z = 0; z < BlockChunk::chunkSizeZ; z++)
            {
                ...
                std::uint32_t blockUpdateCount = stream::read<std::uint32_t>(
                    reader);
                for(; blockUpdateCount > 0; blockUpdateCount--)
                {
                    PositionI position = stream::read<PositionI>(reader);
                    if(chunkBasePosition
                        != BlockChunk::getChunkBasePosition(position))
                        throw stream::InvalidDataValueException(
                            "block update is outside of chunk");
                    float timeLeft = stream::read_limited<float32_t>(reader, 0,
                        1e6);
                    BlockUpdateKind blockUpdateKind = stream::read<
                        BlockUpdateKind>(reader);
                    VectorI subchunkIndex =
                        BlockChunk::getSubchunkIndexFromPosition(position);
        ...
        for(VectorI subchunkPos = VectorI(0);
            subchunkPos.x < BlockChunk::chunkSizeX; subchunkPos.x +=
                BlockChunk::subchunkSizeXYZ)
        {
                ...
                subchunkPos.y += BlockChunk::subchunkSizeXYZ)
            {
                    ...
                    subchunkPos.z += BlockChunk::subchunkSizeXYZ)
                {
                    ...
                    sbi.moveBy(subchunkPos, lock_manager);
                    BlockChunkSubchunk &subchunk = sbi.getSubchunk();
                    VectorI subchunkIndex =
                        BlockChunk::getSubchunkIndexFromChunkRelativePosition(
                            subchunkPos);
                    std::vector<std::tuple<PositionI, float, BlockUpdateKind>> & currentBlockUpdates =
                        blockUpdates[subchunkIndex.x][subchunkIndex.y][subchunkIndex.z];
                    for(auto update : currentBlockUpdates)
                    {
                        BlockIterator bi = sbi;
                        bi.moveTo(std::get<0>(update), lock_manager);
                        world.addBlockUpdate(bi, lock_manager,
                            std::get<2>(update), std::get<1>(update));
                    }
                            ...
                            subchunkRelativePos.y++)
                        {
                                ...
                                subchunkRelativePos.z++)
                            {
                                ...
                                bi.moveBy(subchunkRelativePos,
                                    lock_manager);
                                VectorI newBlocksPosition = subchunkRelativePos
                                    + subchunkPos;
                                Block newBlock =
                                    blocks[newBlocksPosition.x][newBlocksPosition.y][newBlocksPosition.z];
                                BlockChunkBlock &b = bi.getBlock(lock_manager);
                                BlockDescriptorPointer bd =
                                    subchunk.getBlockKind(b);
                                if(bd != nullptr && bd->generatesParticles())
                                {
                                    subchunk.removeParticleGeneratingBlock(
                                        bi.position());
                                }
                                bd = newBlock.descriptor;
                                BlockChunk::putBlockIntoArray(
                                    BlockChunk::getSubchunkRelativePosition(
                                        bi.currentRelativePosition), b,
                                    subchunk, std::move(newBlock),
                                    lock_manager.tls);
                                if(bd != nullptr && bd->generatesParticles())
                                {
                                    subchunk.addParticleGeneratingBlock(
                                        bi.position());
                                }
                            }
                        }
    ...
    float timeOfDayInSeconds = stream::read_limited<float32_t>(reader, 0,
        dayDurationInSeconds);
    std::uint8_t moonPhase = stream::read_limited<std::uint8_t>(reader, 0,
        moonPhaseCount);
...
bool World::isChunkCloseEnoughToPlayerToGetRandomUpdates(
    PositionI chunkBasePosition)
{
    LockedPlayers lockedPlayers = players().lock();
    for(std::shared_ptr<Player> player : lockedPlayers)
    {
        PositionI playerChunkBasePosition = BlockChunk::getChunkBasePosition(
            (PositionI)player->getPosition());
        if(playerChunkBasePosition.d != chunkBasePosition.d)
            continue;
        VectorI displacement = playerChunkBasePosition - chunkBasePosition;
        VectorI v(std::abs(displacement.x), std::abs(displacement.y),
